...
class ColourPickerMethod:
    ...
    async def setup(self) -> None:
        ...
        self.green_ctx = self.green_canvas.getContext("2d", {"willReadFrequently":True})
    ...
    def setup_canvas(self) -> None:
        for canvas, ctx, colour in zip(self.RGB_canvas, self.RGB_ctx, self.RGB):
    ...
    def setup_event_listener(self) -> None:
        def mouse_down(evt) -> None:
            self.is_mouse_down = True

        async def mouse_up(evt) -> None:
        ...
        for canvas in self.RGB_canvas:
            ...
            add_event_listener(canvas, "click", self.update_colour)
    ...
    def update_colour(self, evt) -> None:
        canvas = evt.target
        for idx, colour in enumerate(self.hexcodes.keys()):
            if canvas.id == f"{colour}-canvas":
                pixel = self.RGB_ctx[idx].getImageData(evt.offsetX, evt.offsetY, 1, 1).data
